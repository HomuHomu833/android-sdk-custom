 ...
 * headers pull it in, then provide the 'long long' definitions ourselves.
 * On LP64 MIPS long and long long are both 64-bit, so the ABI is identical;
 * only the type name differs.  All code in a TU therefore sees a consistent
 * 'unsigned long long' for __u64/blk64_t/etc., eliminating both error classes.
 *
